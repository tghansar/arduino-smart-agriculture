...
Aim:
  Would like to see the Yun as the receiver with two Unos as senders
...
{/*
  ...
  }else{                            //Switch between the Unos
      if (selectNode == 1){
        mySerial.print('1');//delay(50);
        selectNode = 2;       //Switch
        delay(1900);          //These delays will help wait for an Uno to do its delay for grabbing Moisture and Temp values.
                              //Make sure that its not too late or else the above else statement encapsulating this block will execute and skip a Node.
      }
      else{
        mySerial.print('2');//delay(50);
        selectNode = 1;       //Switch
        delay(1900); 
      }    
  }
...
/*
//Useful code for telling time, the code is from another source: https://blog.arduino.cc/2013/09/05/hands-on-the-arduino-yuns-bridge/
